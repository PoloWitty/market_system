...
//processing form data wehn form is submitted
if(isset($_POST['id'])&&!empty($_POST['id']))
{
    ...
    {
        $sql = 'UPDATE product SET name=?,address=?,price=?,stockNumber=?,sellerCode=? WHERE id=?';

        if($stmt=mysqli_prepare($link_pd,$sql))
        {
            mysqli_stmt_bind_param($stmt,'ssiiii',$param_name,$param_address,$param_price,$param_stockNumber,$param_sellerCode,$param_id);
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label>Address</label>
                            <p><?php echo($file_address); ?></p>
                            <input type="file" class="form-control <?php echo (!empty($address_err)) ? 'is-invalid' : ''; ?>" tabindex="1" name="afile" value=<?php echo($file_address); ?>>
